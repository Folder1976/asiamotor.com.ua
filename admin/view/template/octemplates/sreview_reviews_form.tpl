      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-review" class="form-horizontal">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <textarea name="admin_text" cols="60" rows="8" placeholder="<?php echo $entry_admin_text; ?>" id="input-admin_text" class="form-control summernote"><?php echo $admin_text; ?></textarea>
            </div>
          </div>
          <div class="form-group">
            <label class="col-sm-2 control-label" for="input-name"><?php echo $entry_avg_rating; ?></label>
            <div class="col-sm-10">
              <label class="radio-inline">
                <?php if ($avg_rating == 1) { ?>
                <input type="radio" name="avg_rating" value="1" checked="checked" />
                1
                <?php } else { ?>
                <input type="radio" name="avg_rating" value="1" />
                1
                <?php } ?>
              </label>
              <label class="radio-inline">
                <?php if ($avg_rating == 2) { ?>
                <input type="radio" name="avg_rating" value="2" checked="checked" />
                2
                <?php } else { ?>
                <input type="radio" name="avg_rating" value="2" />
                2
                <?php } ?>
              </label>
              <label class="radio-inline">
                <?php if ($avg_rating == 3) { ?>
                <input type="radio" name="avg_rating" value="3" checked="checked" />
                3
                <?php } else { ?>
                <input type="radio" name="avg_rating" value="3" />
                3
                <?php } ?>
              </label>
              <label class="radio-inline">
                <?php if ($avg_rating == 4) { ?>
                <input type="radio" name="avg_rating" value="4" checked="checked" />
                4
                <?php } else { ?>
                <input type="radio" name="avg_rating" value="4" />
                4
                <?php } ?>
              </label>
              <label class="radio-inline">
                <?php if ($avg_rating == 5) { ?>
                <input type="radio" name="avg_rating" value="5" checked="checked" />
                5
                <?php } else { ?>
                <input type="radio" name="avg_rating" value="5" />
                5
                <?php } ?>
              </label>
            </div>
...
<script type="text/javascript"><!--
  <?php if ($ckeditor) { ?>
      ckeditorInit('input-admin_text', getURLVar('token'));
  <?php } ?>
