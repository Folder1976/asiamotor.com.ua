...
class ControllerExtensionModuleOct404Page extends Controller {
    private $error = array();

    public function index() {
        $data = array();
        $data = array_merge($data, $this->load->language('extension/module/oct_404_page'));
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/oct_404_page', 'token=' . $this->session->data['token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/module/oct_404_page', 'token=' . $this->session->data['token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/module/oct_404_page.tpl', $data));
    }

    public function install() {
        $this->load->language('extension/module/oct_404_page');
        ...
        $this->model_user_user_group->addPermission($this->user->getId(), 'access', 'extension/module/oct_404_page');
        $this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'extension/module/oct_404_page');

        $results = $this->model_localisation_language->getLanguages();

        foreach ($results as $result) {
            $module_text[$result['language_id']] = array(
                'title' => '',
                'text' => ''
            );
        }

        $this->model_setting_setting->editSetting('oct_404_page', array(
            'oct_404_page_data' => array(
                'status' => '0',
        ...
        if (!in_array('oct_404_page', $this->model_extension_extension->getInstalled('module'))) {
            $this->model_extension_extension->install('module', 'oct_404_page');
        }
    ...
    public function uninstall() {
        ...
        $this->model_extension_extension->uninstall('module', 'oct_404_page');
        $this->model_setting_setting->deleteSetting('oct_404_page');
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/oct_404_page')) {
