...
class ControllerExtensionFeedNeoseoFastSitemap extends NeoseoController
{
	...
	public function index()
	{
		...
		$data = $this->language->load('extension/feed/neoseo_fast_sitemap');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			if (!isset($_GET["close"])) {
				$this->response->redirect($this->url->link('extension/feed/neoseo_fast_sitemap', 'token=' . $this->session->data['token'], 'SSL'));
			} else {
		...
		$data = $this->initBreadcrumbs(array(
			...
			array("extension/feed/neoseo_fast_sitemap", "heading_title_raw")
		    ), $data);


		$data['save'] = $this->url->link('extension/feed/neoseo_fast_sitemap', 'token=' . $this->session->data['token'], 'SSL');
		$data['save_and_close'] = $this->url->link('extension/feed/neoseo_fast_sitemap', 'token=' . $this->session->data['token'] . "&close=1", 'SSL');
		$data['clear'] = $this->url->link('extension/feed/neoseo_fast_sitemap/clear', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/feed/neoseo_fast_sitemap.tpl', $data));
	}

	private function validate()
	{
		if (!$this->user->hasPermission('modify', 'extension/feed/neoseo_fast_sitemap')) {
