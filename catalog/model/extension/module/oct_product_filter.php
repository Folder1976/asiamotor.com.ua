...
class ModelExtensionModuleOctProductFilter extends Model {
    ...
    public function getFilterTotalSpecialOnly($global_id, $global_type, $store_id, $customer_group_id, $language_id, $lead_time_status = false, $data = array()) {
        ...
        if ($product_ids) {
            ...
            if (isset($data['tag']) && !empty($data['tag'])) {
                ...
                $words = explode(' ', trim(preg_replace('/\s+/', ' ', str_replace('_',' ', $data['tag']))));
                ...
                if ($implode_product_tag) {
                    $sql .= " (" . implode(" OR ", $implode_product_tag) . ")";
                }
            }
    ...
    public function getFilterOptions($global_id, $global_type, $store_id, $customer_group_id, $language_id, $lead_time_status = false) {
        ...
        if ($product_ids) {
            ...
            if (!$options) {
                ...
                $query = $this->db->query("SELECT DISTINCT op.sort_order, fpo.option_name, fpo.option_name_mod, fpo.option_id FROM " . DB_PREFIX . "oct_filter_product_option fpo INNER JOIN " . DB_PREFIX . "option AS op ON (fpo.option_id = op.option_id) WHERE fpo.product_id IN (".implode(',', $product_ids).") AND fpo.language_id = '".(int)$language_id."' ORDER BY op.sort_order ASC")->rows;
    ...
    public function getFilterOptionValues($option_name, $global_id, $global_type, $store_id, $customer_group_id, $language_id) {
        ...
        if ($product_ids) {
            ...
            $query = $this->db->query("SELECT fpo.option_value_name, fpo.option_value_name_mod, fpo.option_value_id, fpo.option_value_image FROM " . DB_PREFIX . "oct_filter_product_option fpo LEFT JOIN " . DB_PREFIX . "option_value AS ov ON (fpo.option_value_id = ov.option_value_id)  WHERE fpo.product_id IN (" . implode(',', $product_ids) . ") AND fpo.language_id = '" . (int) $language_id . "' AND fpo.option_name = '" . $this->db->escape($option_name) . "' GROUP BY fpo.option_value_name ORDER BY ov.sort_order ASC, fpo.option_value_name ASC")->rows;
    ...
    public function getFilterTotalStock($global_id, $global_type, $store_id, $customer_group_id, $language_id, $lead_time_status = false, $data = array()) {
        ...
        if ($product_ids) {
            ...
            $stock_check = array(
                ...
                array(
                    'name' => "ending_stock",
                    'query' => "AND (p.quantity BETWEEN '0' AND '" . (int) $data['filter_data']['stock_ending_value'] . "')"
                )
            );
