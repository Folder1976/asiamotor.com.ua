...
class ModelCatalogProduct extends Model {
	...
	public function getProductPrimenimosti($tovarkey){
		
		$sql = "SELECT * FROM " . DB_PREFIX . "car WHERE `key` IN
				(SELECT `modelkey` FROM " . DB_PREFIX . "car_primenimost WHERE tovarkey = '$tovarkey')
				ORDER BY name
				";
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, (SELECT md.name FROM " . DB_PREFIX . "manufacturer_description md WHERE md.manufacturer_id = p.manufacturer_id AND md.language_id = '" . (int)$this->config->get('config_language_id') . "') AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p
			...
			LEFT JOIN " . DB_PREFIX . "product_to_1c prim ON (p.product_id = prim.product_id)
        ...
        'price'            => $this->model_catalog_group_price->updatePrice($query->row, $query->row['discount'] ? $query->row['discount'] : $query->row['price']),
	...
	public function getFilteredProductIds(){
		...
		$sql = "SELECT `key` FROM " . DB_PREFIX . "car WHERE 1 ";
		
		if(isset($this->session->data['mark_id']) AND (int)$this->session->data['mark_id'] > 0){
			$sql .= " AND mark_id='".(int)$this->session->data['mark_id']."'";
		}
		if(isset($this->session->data['model_id']) AND (int)$this->session->data['model_id'] > 0){
			$sql .= " AND model_id='".(int)$this->session->data['model_id']."'";
		}
		if(isset($this->session->data['year_id']) AND (int)$this->session->data['year_id'] > 0){
			$sql .= " AND goda LIKE '%".(int)$this->session->data['year_id'].";%'";
		}
		if(isset($this->session->data['dvigun_id']) AND $this->session->data['dvigun_id'] != '0'){
			$sql .= " AND dvigunkey='".$this->db->escape($this->session->data['dvigun_id'])."'";
		}
		
		$sql_m = "SELECT `tovarkey` FROM " . DB_PREFIX . "car_primenimost WHERE modelkey IN ($sql)";
		
		$sql_t = "SELECT `product_id` FROM " . DB_PREFIX . "product_to_1c WHERE 1c_id IN ($sql_m)";
	...
	public function getProducts($data = array()) {
		...
		if(isset($this->session->data['filtercar'])){
			$products_ids = $this->getFilteredProductIds();

			if(count($products_ids)){
				$sql .= " AND p.product_id IN (" . implode(',', $products_ids) . ")";
			}else{
				$sql .= " AND p.product_id = -1234567";
			}
		}
...
'p.viewed',
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
				$sql .= " ORDER BY (CASE WHEN quantity > 0 THEN 1 ELSE 0 END) DESC, LCASE(" . $data['sort'] . ")";
			} elseif ($data['sort'] == 'p.price') {
				$sql .= " ORDER BY (CASE WHEN quantity > 0 THEN 1 ELSE 0 END) DESC, (CASE WHEN special IS NOT NULL THEN special WHEN discount IS NOT NULL THEN discount ELSE p.price END)";
			} else {
				$sql .= " ORDER BY (CASE WHEN quantity > 0 THEN 1 ELSE 0 END) DESC, " . $data['sort'];
			}
		} else {
...
'p.viewed',
	...
	public function getLatestProducts($limit) {
		$product_data = false; //$this->cache->get('product.latest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id') . '.' . (int)$limit);
	...
	public function getBestSellerProducts($limit) {
		...
		if (!$product_data) {
			...
			$sql = "SELECT op.product_id, SUM(op.quantity) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND p.date_available <= NOW() AND
