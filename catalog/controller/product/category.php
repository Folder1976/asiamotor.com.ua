	...
	public function index() {
      ...
      $oct_product_filter_default_order = str_replace('order=', '', $oct_product_filter_default_sort[1]);
		...
		if ($category_info) {
			...
			if($this->config->get('config_ceo_category_title') != ''){
				$this->document->setTitle(str_replace('@name@',$category_info['name'], $this->config->get('config_ceo_category_title')));
			}
			if($this->config->get('config_ceo_category_description') != ''){
				$this->document->setDescription(str_replace('@name@',$category_info['name'], $this->config->get('config_ceo_category_description')));
			}
			if($this->config->get('config_ceo_category_keywords') != ''){
				$this->document->setKeywords(str_replace('@name@',$category_info['name'], $this->config->get('config_ceo_category_keywords')));
			}
			
			if($this->config->get('config_ceo_category_h1') != ''){
				$data['heading_title'] = str_replace('@name@',$category_info['name'], $this->config->get('config_ceo_category_h1'));
			}
        ...
        $sc_name = str_replace('index.php','', $this->request->server['SCRIPT_NAME']);
		
        //$value_seo_url = preg_replace('/\?sort=.{1,}/', '', $this->request->server['REQUEST_URI']);
        if ($sc_name == '/') {
			$value_seo_url = preg_replace('/\?page=.{1,}/', '', $this->request->server['REQUEST_URI']);
        } else {
	        $value_seo_url = '/'.str_replace($sc_name, '', $this->request->server['REQUEST_URI']);
		...
		foreach ($allowed_parameters as $tr) {
			$value_seo_url = preg_replace('/\?'.$tr.'=.{1,}/', '', $value_seo_url);
			$value_seo_url = preg_replace('/'.$tr.'=.{1,}/', '', $value_seo_url);
		}

		$value_seo_url = preg_replace('/\?utm_/', '', $value_seo_url);
		$value_seo_url = preg_replace('/utm_/', '', $value_seo_url);
		$value_seo_url = preg_replace('/\?openstat_/', '', $value_seo_url);
		$value_seo_url = preg_replace('/openstat_/', '', $value_seo_url);
...
'economy'       => $economy,
        ...
        'product_preorder_status' => $product_preorder_status,
			...
			$limits = array_unique(array($this->config->get($this->config->get('config_theme') . '_product_limit'), 24, 36, 48, 96));
...
$itr = ($this->config->get('config_seo_url') && !isset($oct_link[1])) ? '?' : '&';
