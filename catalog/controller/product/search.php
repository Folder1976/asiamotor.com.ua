...
$data['oct_popup_view_data'] = $this->config->get('oct_popup_view_data');
        ...
        $data['button_popup_view'] = $this->language->get('button_popup_view');
				...
				if (isset($this->request->get['keyword']) && !empty($this->request->get['keyword'])) {
					...
					$implode = array();
					$words = explode(' ', trim(preg_replace('/\s+/', ' ', rawurldecode($this->request->get['keyword']))));
					...
					$sql .= " OR LCASE(p.sku) LIKE '%" . $this->db->escape(utf8_strtolower(rawurldecode($this->request->get['keyword']))) . "%'";
					$sql .= " OR LCASE(p.model) LIKE '%" . $this->db->escape(utf8_strtolower(rawurldecode($this->request->get['keyword']))) . "%'";
					$sql .= ")";
		
		if(isset($this->session->data['filtercar'])){
			
			$products_ids = $this->model_catalog_product->getFilteredProductIds();
			...
			if(count($products_ids)){
				...
				$sql .= " AND p.product_id = -1234567";
					...
					ORDER BY (CASE WHEN quantity > 0 THEN 1 ELSE 0 END) DESC, p.sort_order ASC, LOWER(pd.name) ASC, LOWER(p.model) ASC LIMIT 15";
					...
					foreach ($query->rows as $result) {
						...
						$product_data[] = array(
							...
							'name'    => htmlspecialchars_decode($product_info['name'], ENT_QUOTES),
...
$product_total = 1; //$this->model_catalog_search_mr->getTotalProducts($filter_data);
		...
		if(isset($this->session->data['filtercar'])){
			...
			foreach($results as $index => $row){
				...
				if(!in_array($row['product_id'], $products_ids)){
