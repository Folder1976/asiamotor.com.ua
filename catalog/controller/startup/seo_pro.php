	...
	public function index() {
      ...
      if ($oct_product_filter_status && isset($oct_product_filter_data['enable_seo']) && $oct_product_filter_data['enable_seo'] && (isset($this->request->get['oct_filter']) || preg_match('/filter/', $this->request->server['REQUEST_URI']))) {
        ...
        if (preg_match('/tag-[a-z0-9(_|.)]{1,}/', $this->request->server['REQUEST_URI'])) {
          ...
          $this->cache_data['keywords']['tag-'.$oct_filter_tag[1]] = 'tag='.str_replace(' ','_', $oct_filter_tag[1]);
          $this->cache_data['queries']['tag='.$oct_filter_tag[1]] = 'tag-'.str_replace(' ','_', $oct_filter_tag[1]);
        } elseif (isset($this->request->get['tag'])) {
          $this->cache_data['keywords']['tag-'.$this->request->get['tag']] = 'tag='.str_replace(' ','_',$this->request->get['tag']);
          $this->cache_data['queries']['tag='.$this->request->get['tag']] = 'tag-'.str_replace(' ','_',$this->request->get['tag']);
        } elseif (isset($this->session->data['oct_tag'])) {
          $this->cache_data['keywords']['tag-'.$this->session->data['oct_tag']] = 'tag='.str_replace(' ','_',$this->session->data['oct_tag']);
          $this->cache_data['queries']['tag='.$this->session->data['oct_tag']] = 'tag-'.str_replace(' ','_',$this->session->data['oct_tag']);
        }
        // SEO tag: end
      ...
      // SEO oct product filter: end
		...
		if (!isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $keyword) {
				...
				}elseif (isset($this->cache_data['keywords'][trim($parts[0],'.html')])) {
					//Это для доставки городов
					$tmp = explode('=', $this->cache_data['keywords'][trim($parts[0],'.html')]);

					$this->request->get[$tmp[0]] = $tmp[1];
					if($tmp[0] == 'information_id'){
						$this->request->get['route'] = 'information/information';
					}
					
					$rows[] = array('keyword' => trim($parts[0],'.html'), 'query' => $this->cache_data['keywords'][trim($parts[0],'.html')]);
					
					if(isset($parts[1])){
						$this->request->get['city_alias'] = $parts[1];
					}
