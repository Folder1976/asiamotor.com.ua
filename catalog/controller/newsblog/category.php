...
class ControllerNewsBlogCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			$articles_image_size=array(200, 200); //$this->config->get($this->config->get('config_theme') . '_image_product_width'),$this->config->get($this->config->get('config_theme') . '_image_product_height'));
			...
			if ($category_info['settings']) {
	            ...
	            $articles_image_size=array(110, 110);//$settings['images_size_width'],$settings['images_size_height']);
			...
			if ($limit>0) {
				...
				foreach ($results as $result) {
					...
					$text = mb_substr(trim(html_entity_decode($result['preview'], ENT_QUOTES, 'UTF-8')), 0, 1000);
