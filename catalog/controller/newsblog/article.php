	...
	public function index() {
		...
		if ($article_info) {
			...
			foreach ($results as $result) {
				...
				if ($result['image']) {
					$original 	= HTTP_SERVER.'image/'.$result['image'];
					$thumb = $this->model_tool_image->resize($result['image'], 110, 110); //$this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
				} else {
					$original = false;
					$thumb = false;
				}
